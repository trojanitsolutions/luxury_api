...
class POSClosingEntryOverride:
	...
	def validate_sales_invoices(self):
		"""
		Override Sales Invoice validation to skip user ownership check.
		API layer (luxury_api) handles multi-user/multi-cashier POS closing sessions.

		Validates:
		- Invoice already consolidated
		- Invoice has payments (is_pos)
		- Invoice created using POS (is_created_using_pos)
		- POS Profile match
		- Invoice is submitted

		Skips: user ownership check (ERPNext line 191-194)
		"""
		invalid_rows = []
		...
		for d in self.sales_invoices:
			...
			if sales_invoice.pos_closing_entry:
				invalid_row.setdefault("msg", []).append(frappe._("Sales Invoice is already consolidated"))
				invalid_rows.append(invalid_row)
				continue
			...
			# ponytail: intentionally skip user ownership check
			# ERPNext checks: if sales_invoice.owner != self.user
			# API usage requires invoices from multiple users in one closing session
		...
		if invalid_rows:
			...
			for row in invalid_rows:
				for msg in row.get("msg"):
					error_list.append(frappe._("Row #{}: {}").format(row.get("idx"), msg))
			frappe.throw(error_list, title=frappe._("Invalid Sales Invoices"), as_list=True)
